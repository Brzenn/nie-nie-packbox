(() => {
  ...
  async function detectServer() {
    ...
    for (const base of candidates) {
      try {
        ...
        if (info) {
          ...
          document.getElementById("btn-change-store")?.classList.remove("hidden");
    ...
    document.getElementById("btn-change-store")?.classList.add("hidden");
        ...
        if (job.state && job.state !== "running") {
          ...
          if (job.state === "error") {
            ...
            if (stillHere) {
              ...
              setTimeout(async () => {
                hideProgress();
                const sync = await checkSync(jobRel, true);
  ...
  async function openPath(rel) {
    ...
    async function listLocalDir(pathRel) {
      if (!server) return [];
      const res = await fetch(`${apiBase()}/api/local/list?path=${encodeURIComponent(pathRel || "")}`);
      const data = await res.json();
      return data.entries || [];
    }
    ...
    try {
      let items = [];
      if (mode === "local") {
        items = await listLocalDir(rel);
      } else if (navigator.onLine === false) {
        // 无网络：只读本地已下载
        items = await listLocalDir(rel);
        if (!items.length) {
          els.fileTree.innerHTML = `<div class="tree-empty">无网络，且本地没有该课程的已下载文件</div>`;
          currentItems = [];
          return;
        }
        toast("离线：已显示本地已下载文件", 2500);
      } else {
        try {
          items = await remoteList(rel) || [];
        } catch (e) {
          console.warn("remote list failed, try local", e);
          try {
            items = await listLocalDir(rel);
            if (items.length) toast("已显示本地已下载文件", 2500);
          } catch {
            items = [];
          }
          if (!items.length) {
            els.fileTree.innerHTML = `<div class="tree-empty">无法连接资料库，且本地没有该目录文件</div>`;
            currentItems = [];
            return;
          }
        }
  ...
  async function checkSync(courseRel, force = false) {
    if (!server || !courseRel) return null;
    try {
      const res = await fetch(`${apiBase()}/api/sync/check`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rel: courseRel, force }),
      });
  ...
  async function selectCourse(course) {
    ...
    els.fileStats.textContent = mode === "local" || navigator.onLine === false ? "本地资料" : "检查更新中…";
    els.sizeStats.textContent = "";
    els.btnOpenLocal.classList.add("hidden");
    els.btnDownload.disabled = true;
    els.btnDownload.textContent = "…";
    if (activeJob && activeJob.rel !== course.rel) hideProgress();
    renderCourses();
    await openPath(course.rel);

    if (mode === "local" || navigator.onLine === false) {
      const nFiles = currentItems.filter((x) => !x.is_dir).length;
      const nDirs = currentItems.filter((x) => x.is_dir).length;
      applyDownloadButton({
        has_local: true,
        new_count: 0,
        complete: true,
      ...
      els.fileStats.textContent = `本地 ${nFiles} 个文件 · ${nDirs} 个文件夹`;
      return;
    }

    if (server) {
      applyDownloadButton(null);
      checkSync(course.rel)
        .then((sync) => {
          if (!activeCourse || activeCourse.rel !== course.rel) return;
          applyDownloadButton(sync);
          if (sync) {
            const extra = sync.new_count ? ` · 新增 ${sync.new_count}` : " · 已是最新";
            els.fileStats.textContent = `远端 ${sync.remote_count} · 本地 ${sync.local_count}${extra}`;
        ...
        .catch(() => applyDownloadButton(null));
    } else {
      applyDownloadButton(null);
    }
  }
  ...
  document.getElementById("btn-change-store")?.addEventListener("click", async () => {
    if (!server) {
      toast("请先启动本地服务");
      return;
    }
    ...
    let picked = "";
    try {
      const r = await fetch(`${apiBase()}/api/pick-folder`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "{}",
      });
    ...
    if (!picked) {
      picked = (window.prompt("输入新的课程资源目录绝对路径：", server.resourceRoot || "") || "").trim();
    }
    if (!picked) return;
    const ok = window.confirm(
      `将把本地资料从\n${server.resourceRoot || "课程资源"}\n全部迁移到\n${picked}\n\n是否继续？`
    );
    if (!ok) return;
    try {
      toast("正在迁移资料…");
      const res = await fetch(`${apiBase()}/api/settings/resource-root`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      server.resourceRoot = j.resource_root;
      els.savePath.textContent = `保存到 ${j.resource_root}`;
      toast(`已迁移 ${j.migrated} 个文件到新位置`, 4000);
      // 刷新本地列表
      if (mode === "local") await loadLocalCourses();
    } catch (err) {
      toast(err.message || "迁移失败");
    }
  });
  ...
  (async () => {
    ...
    if (!courses.length) {
      ...
      if (server) toast("网络不可用，已切换到本地课程资源", 2800);
    }
    // 心跳保活；关闭/离开页面时发 goodbye，服务会在约 5 秒后退出
    const beat = () => {
      if (!server) return;
      fetch(`${apiBase()}/api/heartbeat`).catch(() => {});
    };
    const bye = () => {
      if (!server) return;
      const url = `${apiBase()}/api/goodbye`;
      try {
