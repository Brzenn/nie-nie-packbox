...
# 打包成 exe 后资源与配置放在 exe 同目录；源码运行时为脚本目录
if getattr(sys, "frozen", False):
    BASE_DIR = Path(sys.executable).resolve().parent
    ASSET_DIR = Path(getattr(sys, "_MEIPASS", BASE_DIR))
else:
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if path in ("/", "/index.html"):
                ...
                return
            if path == "/viewer.html":
                self._serve_file(ASSET_DIR / "viewer.html")
                return
            if path == "/viewer.js":
                self._serve_file(ASSET_DIR / "viewer.js")
                return
            if path == "/styles.css":
                self._serve_file(ASSET_DIR / "styles.css")
                return
            if path == "/app.js":
                self._serve_file(ASSET_DIR / "app.js")
                return
            if path.startswith("/vendor/"):
                rel = path[len("/vendor/") :]
                fp = (ASSET_DIR / "vendor" / rel).resolve()
                vendor_root = (ASSET_DIR / "vendor").resolve()
...
def _try_existing(url: str) -> bool:
    ...
    try:
        with urllib.request.urlopen(url + "/api/ping", timeout=0.35) as r:
...
def main() -> None:
    ...
    for p in range(8765, 8775):
        if not port_free(p) and _try_existing(f"http://{HOST}:{p}"):
